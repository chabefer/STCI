...
### Using effect sizes to normalize the reporting of treatment effects and their precision
...
for (k in 1:length(N.sample)){
  ...
  plot.test.ES <- ggplot(test.ES, aes(x=as.factor(id), y=ES)) +
      ...
      geom_hline(aes(yintercept=delta.y.ate(param)/sqrt(V0(param))), colour="#990000", linetype="dashed")+
      #ylim(-0.5,1.2)+
      xlab("Sample id")+
      ylab("Effect Size")+
      theme_bw()+
...
We can now graph our bootstrapped estimate of sampling noise in all of our samples, the average bootstrapped estimates of sampling noise and of confidence intervals, in Figures \@ref(fig:sampnoisewwfisherall), \@ref(fig:sampnoisewwfisherplot) and \@ref(fig:confintervalfisher) show.

```{r sampnoisewwfisherall,eval=TRUE,echo=TRUE,fig.cap='Distribution of the estimates of sampling noise using Fisher permutation approach over replications of samples of different sizes (true sampling noise in red)',fig.align='center',out.width=cst,fig.pos='htbp'}
par(mfrow=c(2,2))
for (i in 1:4){
...
```{r sampnoisewwfisherplot,eval=TRUE,echo=TRUE,fig.cap='Average estimates of sampling noise using Fisher permutation approach over replications of samples of different sizes (true sampling noise in red)',fig.align='center',out.width=cst,fig.pos='htbp'}
for (k in (1:length(N.sample))){
...
```{r confintervalfisher,eval=TRUE,echo=TRUE,results='hide',warning=FALSE,error=FALSE,message=FALSE,fig.cap=paste('Confidence intervals of $\\hat{WW}$ for $\\delta=$',delta,'estimated using Fisher permutation approach over sample replications for various sample sizes (true confidence intervals in red)',sep=' '),fig.align='center',out.width=cst,fig.pos='htbp',fig.height=12,fig.width=10}
N.plot <- 40
plot.list <- list()
...
**TO DO: COMMENT AND USE ALTERNATIVE APPROACH USING p-VALUES** 
