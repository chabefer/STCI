...
the sample. Without conditioning on \(Y_i^B\), we are thus making a very
large mistake as to the true ITE. This is probably because \(y_i^B\) is
not identically distributed between treatment and control, generating
sampling noise.
...
\begin{align}
  \theta_k & = \beta'X_i + u_i + e_i,
\end{align}

where \(X_i\) is a vector of variables suspected to be correlated with
treatment effect heterogeneity and \(\beta\) is a vector of
coefficients, of the same dimension as \(X_i\). \(X_i\) contains a \(1\)
as its first term, so that \(\beta_0\), the first component of the
vector \(\beta\) measures the effect of the treatment when all other
regressors are set to zero. It might thus be a good idea to set the
regressors as deviations around their means if we want \(\beta_0\) to
capture the average effect of the treatment. The error terms \(u_i\) and
\(e_i\) capture the remaining heterogeneity in effect sizes due to
remaining unobserved covariates (\(u_i\)) and estimation error due to
sampling noise (\(e_i\)). It is in addition generally assumed that
\(e_i\sim\mathbf{N}(0,\sigma^2_i)\) and \(u_i\sim\mathbf{N}(0,\tau^2)\).

This model is in general called the \textbf{mixed effects linear model}.
It contains at the same time fixed effects captured by \(\beta'X_i\) and
random effects captured by \(u_i\). Setting \(\tau^2\) to zero generates
a \textbf{fixed effects linear model}. It is possible, as usual, to test
for whether \(\tau^2\) is null or not, which is a test of whether the
added covariates fully capture the heterogeneity in treatment effects
across studies.

\subsubsection{Estimating the meta-regression
model}\label{estimating-the-meta-regression-model}

There are basically four ways to estimate the meta-regression model:
