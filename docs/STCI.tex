...
standard errors without conditioning on covariates first.
\EndKnitrBlock{example}
...
\ref{def:randeligvalid}, the Bloom esimator is equal to the ITE divided
...
Estimating samploing noise in the Randomization After Eligibility design
is similar to the estimation of sampling noise in the Encouragement
design. Since the Randomization After Eligibility design is a special
case of the Encouragement design, I detail the estimation of sampling
noise in both designs in Section \ref{sec:design4}.
...
\BeginKnitrBlock{lemma}[Wald and Bloom are IV]
\protect\hypertarget{lem:WaldIV}{}{\label{lem:WaldIV} \iffalse (Wald and
Bloom are IV) \fi{} }Under the assumption that there is at least one
individual with \(R_i=1\) and one individual with \(D_i=1\), the
coefficient \(\beta\) in the following regression estimated using
\(R_i\) as an IV:
...
\NormalTok{cum.test}\OperatorTok{$}\NormalTok{cum.se.ES <-}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(cum.test}\OperatorTok{$}\NormalTok{cum.var)}
  ...
  \KeywordTok{ggplot}\NormalTok{(cum.test, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x=}\KeywordTok{as.factor}\NormalTok{(id), }\DataTypeTok{y=}\NormalTok{cum.ES)) }\OperatorTok{+}
\StringTok{      }\KeywordTok{geom_bar}\NormalTok{(}\DataTypeTok{position=}\KeywordTok{position_dodge}\NormalTok{(), }\DataTypeTok{stat=}\StringTok{"identity"}\NormalTok{, }\DataTypeTok{colour=}\StringTok{'black'}\NormalTok{) }\OperatorTok{+}
...
{\centering \includegraphics[width=0.6\linewidth]{STCI_files/figure-latex/cumWMAE-1} 

}

\caption{Example data set: constantly updated meta-analysis}\label{fig:cumWMAE}
\end{figure}
...
estimate with a larger sample size (\(N=\) 1000): 0.17 \(\pm\) 0.18.
Both estimates actually have the exact same precision (because they
actually have the same sample size). The same is true when combining the
first 17 studies. The meta-analytic effect is estimated at 0.24 \(\pm\)
0.06, while the effect estimated using one unique RCT with a larger
sample size is (\(N=\) 10\^{}\{4\}) 0.21 \(\pm\) 0.05. Finally, the same
result occurs when combining the first 19 studies. The meta-analytic
effect is estimated at 0.21 \(\pm\) 0.03, while the effect estimated
using one unique RCT with a larger sample size is (\(N=\) 10\^{}\{5\})
0.19 \(\pm\) 0.02.
