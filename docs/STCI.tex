...
\caption{Distribution of the estimates of sampling noise using Fisher permutation approach over replications of samples of different sizes (true sampling noise in red)}\label{fig:sampnoisewwfisherall}
\end{figure}
...
\caption{Average estimates of sampling noise using Fisher permutation approach over replications of samples of different sizes (true sampling noise in red)}\label{fig:sampnoisewwfisherplot}
\end{figure}
...
\caption{Confidence intervals of $\hat{WW}$ for $\delta=$ 0.99 estimated using Fisher permutation approach over sample replications for various sample sizes (true confidence intervals in red)}\label{fig:confintervalfisher}
\end{figure}

\textbf{TO DO: COMMENT AND USE ALTERNATIVE APPROACH USING p-VALUES}
...
\textbf{proof needed, especially assumption of linearity.}
\textbf{Also, is interaction between \(X_i\) and \(R_i\) needed? See lecture 3.}
