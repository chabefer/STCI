...
We also have two potential outcomes for \(D_i\): \(D_i^{r}\),
\(r\in\left\{0,1\right\}\). \(D_i^{1}\) indicates whether individual
\(i\) takes the treatment when receving the encouragement and
\(D_i^{0}\) whether he takes the treatment when not receiving the
...
\tightlist
\item
  \textbf{Always takers}, that take up the program whether they receive
  the encouragement or not. They are such that \(D_i^{1}=D_i^{0}=1\). I
  denote them \(T_i=a\).
\item
  \textbf{Never takers}, that do not take up the program whether they
  receive the encouragement or not. They are such that
  \(D_i^{1}=D_i^{0}=0\). I denote them \(T_i=n\).
\item
  \textbf{Compliers}, that take up the program when they receive the
  encouragement and do not when they do not receive the encouragement.
  They are such that \(D_i^{1}-D_i^{0}=1\). I denote them \(T_i=c\).
\item
  \textbf{Defiers}, that do not take up the program when they receive
  the encouragement and take it up when they do not receive the
...
\end{align*}
\EndKnitrBlock{definition} with \(Y_i^{1,1}\), \(Y_i^{0,1}\),
\(Y_i^{0,0}\), \(Y_i^{1,0}\), \(D_i^1\) and \(D^0_i\) the same potential
...
\fi{} }We assume that the encouragement increase participation for
everyone:
...
\ref{def:ExclRestr}, \ref{def:Fstage}, \ref{def:Mono}, the Wald
...
with (keeping the conditioning on \$E\_i=1 implicit):
