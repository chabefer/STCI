...
\textbf{\textsc{Another approach coud use the distance between the observed pp-curve of the previous section and the pp-curve with various levels of power. Once the power is identified, the effect size is identified. Still another approach would be to use the standardized distribution of effects (distributed as a standardized normal) to estimate the mean of the distribution and then recover the treatment effect. }}
...
\NormalTok{  ppCurve.grid <-}\StringTok{ }\KeywordTok{sapply}\NormalTok{(thetac.grid,ppCurveEst,}\DataTypeTok{thetak=}\NormalTok{thetak,}\DataTypeTok{sigmak=}\NormalTok{sigmak,}\DataTypeTok{alpha=}\NormalTok{alpha)}
  \CommentTok{# compute sht KS stat for each value of thetac (over columns)}
...
\NormalTok{  min.theta.c <-}\StringTok{ }\NormalTok{thetac.grid[}\KeywordTok{match}\NormalTok{(}\KeywordTok{min}\NormalTok{(KS.grid),KS.grid)]}
  ...
  \KeywordTok{return}\NormalTok{(}\KeywordTok{list}\NormalTok{(min.theta.c,thetac.grid,KS.grid,}\KeywordTok{ecdf}\NormalTok{(ppCurve.grid[,}\KeywordTok{match}\NormalTok{(}\KeywordTok{min}\NormalTok{(KS.grid),KS.grid)])))}
\NormalTok{\}}

\NormalTok{test <-}\StringTok{ }\KeywordTok{ppCurveEstES}\NormalTok{(}\DataTypeTok{thetak=}\KeywordTok{c}\NormalTok{(}\DecValTok{4}\NormalTok{,}\DecValTok{2}\NormalTok{,}\DecValTok{3}\NormalTok{,}\DecValTok{1}\NormalTok{),}\DataTypeTok{sigmak=}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{,}\FloatTok{0.5}\NormalTok{),}\DataTypeTok{thetacl=}\DecValTok{0}\NormalTok{,}\DataTypeTok{thetach=}\DecValTok{10}\NormalTok{,}\DataTypeTok{alpha=}\FloatTok{0.05}\NormalTok{,}\DataTypeTok{ngrid=}\DecValTok{100}\NormalTok{)}

\KeywordTok{plot}\NormalTok{(test[[}\DecValTok{2}\NormalTok{]],test[[}\DecValTok{3}\NormalTok{]])}
\end{Highlighting}
\end{Shaded}

\includegraphics{STCI_files/figure-latex/ppCurveEst-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(test[[}\DecValTok{4}\NormalTok{]])}
\KeywordTok{curve}\NormalTok{(punif,}\DataTypeTok{add=}\NormalTok{T)}
\end{Highlighting}
\end{Shaded}

\includegraphics{STCI_files/figure-latex/ppCurveEst-2.pdf}
