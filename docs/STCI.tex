...
\BeginKnitrBlock{remark}
\iffalse{} {Remark. } \fi{}Since Monotonicity is going to play such a
particular role, and since we have already explored this assumption a
little in Chapter \ref{RCT}, I am going to use as an example a model
where the Monotonicity assumption actualluy does not hold. It will, I
hope, help us understand better the way Monotonicity works and how it
interacts with other assumptions. The key component of the model that
makes Monotonicity necessary is the fact that treatment effects are
heterogeneous and correlated with participation in the treatment. We'll
see later that Monotonicity is unnecessary when treatment effects are
orthogonal to take up.
\EndKnitrBlock{remark}
...
\textbf{\textsc{Give the complete model (\(Y_i\) as well).}}
...
\tightlist
\item
  \textbf{Always takers} (\(T_i=at\)): individuals that participate in
  the program both when \(Z_i=0\) and \(Z_i=1\).
\item
  \textbf{Never takers} (\(T_i=nt\)): individuals that do not
  participate in the program both when \(Z_i=0\) and \(Z_i=1\).
\item
...
In our model, these four types are a funtion of \(V_i\) and
...
three types defined by where \(V_i\) lies with respect to a series of
thresholds:

\begin{itemize}
\tightlist
\item
  \textbf{Always takers} (\(T_i=at\)) are such that
  \(D^1_i=\uns{y_i^B-\bar{\kappa} + V_i\leq\bar{y}}=1\) and
  \(D^0_i=\uns{y_i^B + V_i\leq\bar{y}}=1\), so that they actually are
  such that: \(V_i\leq\bar{y}-y_i^B\). This is because
  \(V_i\leq\bar{y}-y_i^B \Rightarrow V_i\leq\bar{y}-y_i^B+\bar{\kappa}\),
  when \(\bar{\kappa}>0\).
\item
  \textbf{Never takers} (\(T_i=nt\)) are such that
  \(D^1_i=\uns{y_i^B-\bar{\kappa} + V_i\leq\bar{y}}=0\) and
  \(D^0_i=\uns{y_i^B + V_i\leq\bar{y}}=0\), so that they actually are
  such that: \(V_i>\bar{y}-y_i^B+\bar{\kappa}\). This is because
  \(V_i>\bar{y}-y_i^B+\bar{\kappa} \Rightarrow V_i\leq\bar{y}-y_i^B\),
  when \(\bar{\kappa}>0\).
\item
  \textbf{Compliers} (\(T_i=c\)) are such that
  \(D^1_i=\uns{y_i^B-\bar{\kappa} + V_i\leq\bar{y}}=1\) and
  \(D^0_i=\uns{y_i^B + V_i\leq\bar{y}}=0\), so that they actually are
  such that: \(\bar{y}-y_i^B<V_i\leq\bar{y}-y_i^B+\bar{\kappa}\).
\end{itemize}
...
\item
  ...
  \(D^0_i=\uns{y_i^B + V_i\leq\bar{y}}=1\), so that they actually are
  such that: \(V_i\leq\bar{y}-y_i^B-\underline{\kappa}\). This is
  because
  \(V_i\leq\bar{y}-y_i^B-\underline{\kappa} \Rightarrow V_i\leq\bar{y}-y_i^B\),
  when \(\underline{\kappa}>0\).
\item
  \textbf{Never takers} (\(T_i=nt\)) are such that
  \(D^1_i=\uns{y_i^B-\bar{\kappa} + V_i\leq\bar{y}}=0\) and
  \(D^0_i=\uns{y_i^B + V_i\leq\bar{y}}=0\), so that they actually are
  such that: \(V_i>\bar{y}-y_i^B\). This is because
  \(V_i>\bar{y}-y_i^B \Rightarrow V_i\leq\bar{y}-y_i^B-\underline{\kappa}\),
  when \(\underline{\kappa}>0\).
\item
  \textbf{Defiers} (\(T_i=c\)) are such that
  \(D^1_i=\uns{y_i^B+\underline{\kappa} + V_i\leq\bar{y}}=0\) and
  \(D^0_i=\uns{y_i^B + V_i\leq\bar{y}}=1\), so that they actually are
  such that: \(\bar{y}-y_i^B-\underline{\kappa}<V_i\leq\bar{y}-y_i^B\).
\end{itemize}

\textbf{\textsc{Give two plots for V\_i as a funciton of the thresholds,
depending on the vaueof kappa\_i, showing the types. Don't forget to put
the whole model above.}}

Let's generate some data according to this process. For that, let's
choose values for the new parameters.
...
\NormalTok{kappa <-}\StringTok{ }\KeywordTok{ifelse}\NormalTok{(xi}\OperatorTok{==}\DecValTok{1}\NormalTok{,param[}\StringTok{"barkappa"}\NormalTok{],param[}\StringTok{"underbarkappa"}\NormalTok{])}
...
\(Z_i=1\) in our sample is equal to 0.56 while the proportion of treated
when \(Z_i=0\) is equal to 0.27, in accordance with Assumption
\ref{def:FirstStage}.
...
\subsection{Meta-analysis as a weighted average}\label{MetaWA}
...
heterogeneity into account}\label{sec:RE}
...
counting the numbet of studies that fall under one of three categories:
...
\ref{Power}, we derive a formula for the power of the test:

\textbf{\textsc{Continue, assuming unit variance because standardized
effect size.}}
