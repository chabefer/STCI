...
In a Randomization After Eligiblity, we need three assumptions to ensure
...
captures the effect of annoucing eligibility on those who decide to
...
treatment not on all the treated but on those treated whose
participation into the program has been triggered by the encouragement.
These individuals reacting to the encouragement by participating in the
...
identification for Encouragement designs is that a Wald ratio (an IV
estimator) recovers the LATE. It is due to Imbens and Angrist (1994). A
key assumption for this result is exclusion restriction: there has to be
zero impact of the encouragement on the outcome, except through
...
\subsection{Identification of the Local Average Treatment
...
with (keeping the conditioning on \(E_i=1\) implicit):

\begin{align*}
  \Delta^Y_{Wald} & = \frac{\esp{Y_i|R_i=1} - \esp{Y_i|R_i=0}}{\Pr(D_i=1|R_i=1)-\Pr(D_i=1|R_i=0)}\\
  \Delta^Y_{LATE} & = \esp{Y^1_i-Y^0_i|T_i=c}.
\end{align*}

\BeginKnitrBlock{proof}
\iffalse{} {Proof. } \fi{}Under Assumptions \ref{def:RandEncouragValid},
\ref{def:IndepEncourag} and \ref{def:ExclRestr}, we have:

\begin{align*}
\esp{Y_i|R_i=1} & = \esp{Y_i^1|T_i=a}\Pr(T_i=a)+ \esp{Y_i^1|T_i=c}\Pr(T_i=c) \\
                            & \phantom{=} + \esp{Y_i^0|T_i=d}\Pr(T_i=d)+ \esp{Y_i^0|T_i=n}\Pr(T_i=n)\\
\esp{Y_i|R_i=0} & = \esp{Y_i^1|T_i=a}\Pr(T_i=a)+ \esp{Y_i^0|T_i=c}\Pr(T_i=c) \\                     
                                & \phantom{=} + \esp{Y_i^1|T_i=d}\Pr(T_i=d)+ \esp{Y_i^0|T_i=n}\Pr(T_i=n),
\end{align*}

As a consequence:

\begin{align*}
\esp{Y_i|R_i=1}- \esp{Y_i|R_i=0} & = \esp{\Delta^Y_i|T_i=c}\Pr(T_i=c)-\esp{\Delta^Y_i|T_i=d}\Pr(T_i=d)\\
                                 & = \Delta^Y_{LATE}\Pr(T_i=c),
\end{align*}

where the second equality uses Assumption \ref{def:Mono} and the
definition of \(LATE\).

We also have:

\begin{align*}
\Pr(D_i=1|R_i=1) & = \Pr(T_i=a)+ \Pr(T_i=c) \\
\Pr(D_i=1|R_i=0) & = \Pr(T_i=a)+ \Pr(T_i=d),
\end{align*}

where we have used Assumptions \ref{def:RandEncouragValid} and
\ref{def:IndepEncourag}. As a consequence, under Assumption
\ref{def:Mono}, we have:

\begin{align*}
\Pr(D_i=1|Z_i=1)-\Pr(D_i=1|Z_i=0) & = \Pr(T_i=c).
\end{align*}
...
\BeginKnitrBlock{remark}
\iffalse{} {Remark. } \fi{}The intuitution for the proof of Theorem
\ref{thm:IdentLATE} is as follows:
\EndKnitrBlock{remark}

\begin{itemize}
\tightlist
\item
  In the treatment arm (with \(R_i=1\)), everyone receives the
  encouragement to enter the program. Only always takers and compliers
  actually take the program, and we thus see \(Y^1_i\) for them. Never
  takers and defiers do not take the program, and we thus see \(Y^0_i\)
  for them.
\item
  In the control arm (with \$R\_i=0), no one receives the encouragement.
  Only always takers and defiers take the program, and we thus see
  \(Y^1_i\) for them. Never takers and compliers do not take the
  program, and we thus see \(Y^0_i\) for them.
\item
  When comparing the average outcomes between the treatment and control
  arms, we thus have two terms that cancel, since they are the same on
  both arms: the outcomes of the always treated and the outcomes of the
  never treated. We are thus left with the impact on the compliers minus
  the impact on the defiers, both weighted by theirn proportion in the
  population. If there are no defiers, we see the impact on the
  compliers multiplied by their proportion.
\item
  The difference between the proportion of participants in the treatment
  and control arms estimates the proportion of compliers minus the
  proportion of defiers. Under monotonicity, the proportion of defiers
  is zero, and thus the difference between the proportion of
  participants in the treatment and control arms estimates the
  proportion of compliers.
\end{itemize}

\BeginKnitrBlock{remark}
\iffalse{} {Remark. } \fi{}In the proof of Theorem \ref{thm:IdentLATE},
I use the types random variable \(T_i\) instead of the potential
outcomes \(D^1_i\) and \(D^0_i\). This is equivalent, since \(T_i\) is a
function of \(D^1_i\) and \(D^0_i\). Assumption \ref{def:IndepEncourag}
especially is also true when replacing the potential outcomes \(D^1_i\)
and \(D^0_i\) by \(T_i\). This follows from Lemma 4.2 in Dawid (1979)
that shows that if \(X\Ind Y|Z\) and \(U\) is a function of \(X\) then
\(U\Ind Y|Z\).
\EndKnitrBlock{remark}

\BeginKnitrBlock{remark}
\iffalse{} {Remark. } \fi{}The intuition for Theorem \ref{thm:IdentLATE}
is that the encouragement only generates the entry of compliers in the
program. We are thus only able to recover the effect of the treatment on
the compliers. The experimental variation is not strong enough so as to
enable us to estimate the impact on always takers or on never takers.
\EndKnitrBlock{remark}

\BeginKnitrBlock{remark}
\iffalse{} {Remark. } \fi{}The problem with LATE is that it is not a
very useful parameter for evaluating the treatment. LATE is the impact
of the treatment on an unobserved subpopulation of individuals. To
generalize from the LATE to the TT, we can make the assumption that the
impact on always takers is equal to the impact on compliers, but that
...
\textbf{\textsc{Develop intuition of ITE rigorously as in RAE. }}
...
\item
  ...
  The same is true for Job Training Programs, Payments for
  Environmenetal Services, microcredit, graduation programs, nudges,
  etc. Actually, most programs that economists study differ from one
  implementation to the next. In psychology and medecine, most
  treatments are accompanied by a rigorous protocol that makes them much
  more homogeneous.
\item
