...
\EndKnitrBlock{remark} The assumptions needed for that to happen are
intuitive:
...
\item
  ...
  treatment, but only one flow in (Assumption \ref{def:Mono})
\end{enumerate}

Under these assumptions, the only way that we see a difference in
...
\item
  ...
  the same way to the treatment as the compliers. They probably do not,
  since the expected benefits of the treatment might enter in the
  decision of taking it up.
\item
...
informatio on some subpopulation to infer what would happen in another.
Comparing LATEs and TTs for similar treatments is an active area for
reasearch. I know of no paper doing that extensively and nicely.
...
estimator, you're correct.
\EndKnitrBlock{remark} The Wald estimator is actually an IV estimator
(see Lemma \ref{lem:WaldIV} below).
...
\item
  Some individuals (the defiers) decide to exit the treatment,
\item
...
\item
  ...
  the LATE under Assumption \ref{def:ExclRestr}, when there is no direct
  effect of the encouragement on outcomes.
\item
...
All these effects are weighted by the respective proportions of types in
the population.

ITE ``contains'' LATE, as we have already seen, but that can be made
clearer:
...
\begin{align*}
  \Delta^Y_{ITE} & = \esp{Y_i^{1}-Y_i^{0}|T_i=c,E_i=1}\Pr(T_i=c|E_i=1)-\esp{Y_i^{1}-Y_i^{0}|T_i=d,E_i=1}\Pr(T_i=d|E_i=1).
\end{align*}
\EndKnitrBlock{theorem}
...
\begin{align*}
  \Delta^Y_{ITE} & = \esp{Y_i^{1}-Y_i^{1}|T_i=a,E_i=1}\Pr(T_i=a|E_i=1)\nonumber\\
                ...
                & \phantom{=}+ \esp{Y_i^{0}-Y_i^{0}|T_i=n,E_i=1}\Pr(T_i=n|E_i=1),\label{eq:ITE3}
\end{align*}
...
down any direct effect of ereceiving the encouragement on outcomes. As a
...
Hence, ITE is equal to the impact of the treatment on those who react to
...
is equal to the LATE multiplied by the proportions of compliers in the
population:
...
\begin{align*}
  \Pr(T_i=1-c|E_i=1) & = \Pr(D_i=1|R_i=1,E_i=1)-\Pr(D_i=1|R_i=0,E_i=1).
\end{align*}
\EndKnitrBlock{theorem}
...
between the two treatment arms. The parts due to \emph{always takers}
and \emph{never takers} cancel under Assumptions \ref{def:IndepEncourag}
and \ref{def:ExclRestr}. But this cancelling out only happens in the
...
the effect of the encouragement on \emph{always takers} and \emph{never
takers} is null under assumption \ref{def:ExclRestr}, but that would
require observing the type variable \(T_i\). Unfortunately, we cannot
now the type of each individual in the sample, since it is defined
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tau <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{0.1}\NormalTok{,}\FloatTok{0.2}\NormalTok{)}
...
estimates of \(\theta\), one with a standard deviation of 0.1 and the
other of 0.2. Let's visualize our two new datasets:
...
{\centering \subfloat[$\tau^2=$ 0.1\label{fig:metanoiseplot1}]{\includegraphics[width=0.5\linewidth]{STCI_files/figure-latex/metanoiseplot-1} }\subfloat[$\tau^2=$ 0.2\label{fig:metanoiseplot2}]{\includegraphics[width=0.5\linewidth]{STCI_files/figure-latex/metanoiseplot-2} }
...
Hedges' estimates of \(\tau^2\) in our examples are thus -0.03 and 0.04
respectively. The second one is equal to the truth, which is 0.04. The
first one is still imprecise.
