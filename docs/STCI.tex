...
}Under Independence and Validity of the Instrument, Exclusion
Restriction, and Full Rank, we have:
...
Under Monoticity, the variance of the Wald estimator can be decomposed
as follows:
...
}Let's see how the Wald estimator performs in our example:
\EndKnitrBlock{example} Without conditioning on \(y_i^B\):
...
\item
  99\% sampling noise estimated using default 2SLS standard errors is
  2.124
\item
  99\% sampling noise estimated using heteroskedasticity robust 2SLS
  standard errors is 2.119.
\end{itemize}

With conditioning on \(y_i^B\):
...
\ref{sec:RE}, it is using as weigths not only the precision estimates
...
From Lemma \ref{thm:asympIV}, we know that
