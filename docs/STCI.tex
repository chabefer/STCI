...
\begin{align*}
  f_{\hat{Z}|Z}(\hat{z}|z) & = f_{\hat{Z}^*|Z^*,D=1}(\hat{z}|z)\\
                           & = \frac{\Pr(D=1|\hat{Z}_k^*=\hat{z},Z_k^*=z)}{\Pr(D=1|Z_k^*=z)}\phi(\hat{z}-z)\\
...
\protect\hypertarget{prp:AKident}{}{\label{prp:AKident} \iffalse (Identification of the true effect in meta-analysis) \fi{} }Under the assumption that \(\theta^*_k\Ind\sigma^*_k\), and that the support of \(\sigma_k\) contains an open interval, \(p(.)\) is identified up to scale and the distribution of \(\theta\) is identified.
\EndKnitrBlock{proposition}

\BeginKnitrBlock{proof}
\iffalse{} {Proof. } \fi{}See Andrews and Kasy, \href{supplemtary material}{https://www.aeaweb.org/content/file?id=10286}.
\EndKnitrBlock{proof}

\BeginKnitrBlock{remark}
\iffalse{} {Remark. } \fi{}There might be a mistake in the proof of Proposition 3.
Equation (2) states that the derivative of the density with respect to precision is equal to a constant plus a unique term function of the density of latent variables.
Im pretty sure there is another term accounting for the probability of publication as a function of precision.
Whether this kills identification or not remains to be seen.
\EndKnitrBlock{remark}
