...
The key technical intuition behind the p-curve approach to correction for publication bias is to notice that the pp-curve computed for the true treatment effect shold be uniform.
...
Let's assume that we are only looking at statistically significant results for two-sided t-tests, but that are located on the positive side of the threshold: \(\frac{\hat{\sigma}_k}{\hat{\theta}_k}\geq\Phi^{-1}\left(1-\frac{\alpha}{2}\right)\).
...
\protect\hypertarget{thm:ppvalueHomogPos}{}{\label{thm:ppvalueHomogPos} \iffalse (pp-value with homogeneous positive treatment effect) \fi{} }Under the assumption that the treatment effect is homogeneous across studies and equal to \(\theta_c\), that the estimated effects \(\hat{\sigma}_k\) are approximately normally distributed with mean \(\theta_c\) and standard error \(\hat{\theta}_k\) and that we use only effects are positive and significant at the level \(\alpha\) following a two-sided test, the pp-value of a result with estimated effect \(\hat{\sigma}_k\) and estimated standard error is:
...
The fifth equality uses the usual propertyf of the cumulative of the standardized and centered normal distribution that \(1-\Phi(x)=\Phi(-x)\).
\EndKnitrBlock{proof}

First, we know that the p-value stems from a test assuming \(\theta_c=0\).
As a consequence, \(p_0(\hat{\theta}_k,\hat{\sigma}_k)\), the p-value under the two-sided null of a zero effect can be found using the following formula:

\begin{align*}
  p_0(\hat{\theta}_k,\hat{\sigma}_k) & = 2\left(1-\Phi\left(\left|\frac{\hat{\theta}_k}{\hat{\sigma}_k}\right|\right)\right).
\end{align*}

The pp-value is the probability of observing this p-value or a smaller one if the true effect is \(\theta_c\), conditional on observing a statistically significant effect (\textit{i.e.} \(p_0(\hat{\theta}_k,\hat{\sigma}_k)\leq0.05\)).

invert the p-value under the null to recover the t-statistic, and then to transform the test statistic by using the hypothetized true value and finally to reform a new p-value for this modified distribution and to divide it by the probability of obtaining a significant treatment effect (\textit{a.k.a.} power).

\begin{align*}
  \left|\frac{\hat{\theta}_k}{\hat{\sigma}_k}\right| & = \Phi^{-1}\left(1-\frac{p_0(\hat{\theta}_k,\hat{\sigma}_k)}{2}\right).
\end{align*}

One problem with this approach is that you have to be very careful with the sign of the estimated treatment effect.
It also has the problem of going to p-values from treatment effects, and then move back to treatment effects to go to pp-values.
A simpler approach is to eschew the p-value and
