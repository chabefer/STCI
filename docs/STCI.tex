...
\subsection{The with/without comparison, selection bias and
cross-sectional
confounders}\label{the-withwithout-comparison-selection-bias-and-cross-sectional-confounders}
...
\subsubsection{The with/without
comparison}\label{the-withwithout-comparison}
...
}Selection bias is the difference between the counterfactual expected
potential outcome in the absence of the treatment among the treated and
the expected potential outcome in the absence of the treatment among the
untreated.
...
\EndKnitrBlock{example} \[
  \Delta^y_{SB} = \Delta^y_{WW}-\Delta^y_{TT}= -1.298 - 0.172 = `round(delta.y.ww(param)-delta.y.tt(param),3)`
\]

We could have computed \(SB\) directly using the formula from
Theorem\textasciitilde{}\ref{th:SB}:
