...
\item
  ...
  for them.
\item
  In the control arm (with \(R_i=0\)), no one receives the
  encouragement. Only always takers and defiers take the program, and we
  thus see \(Y^1_i\) for them. Never takers and compliers do not take
  the program, and we thus see \(Y^0_i\) for them.
\item
...
and \(D^0_i\) by \(T_i\). This follows from Lemma 4.2 in
\href{https://www.jstor.org/stable/2984718}{Dawid (1979)} that shows
that if \(X\Ind Y|Z\) and \(U\) is a function of \(X\) then
\(U\Ind Y|Z\).
\EndKnitrBlock{remark}
...
Effect}\label{identification-of-the-intention-to-treat-effect}
...
\textbf{\textsc{Show Hedges reweighted estimator of average treatment
effect.}}

\textbf{\textsc{Show Hedges estimator of variance of treatment effect
...
equality follows from the fact that \((D_i^0=1\cup D_i^0=0)\) is a
partition of the sample space. The third equality follows from usual
rules of logic and the fourth equality from the fact that \(D_i^1\) and
\(D_i^0\) can only take values zero and one. The fifth equality follows
from the definitiin of \(T_i\). The sixth equaity follows from the rule
of addition in probability and the fact that \(T_i=a\) and \(T_i=c\) are
disjoint. The final equality follows from Lemma \ref{lem:UnconfTypes}.
