...
\item
  We can run a regression of \(Y\) on \(D\) using \(R\) as an
  instrumental variable and controlling for some variables \(X_i\).
\end{enumerate}
...
\textbf{\textsc{Write Wald}}

\BeginKnitrBlock{example}
\protect\hypertarget{exm:unnamed-chunk-121}{}{\label{exm:unnamed-chunk-121}
}Let's check that these results hold in our numerical example.
\EndKnitrBlock{example}
...
We already know that the estimated ITE is equal to 0.179, which is equal
to the numerator of the Wald estimator. We also now that the proportion
of compliers in our sample is equal to 0.213. As a consequence, again,
the Wald estimator of the LATE is equal to 0.179 \(\div\) 0.213 \(=\)
0.841.
...
\textbf{\textsc{Example}} \textbf{\textsc{Center covariates at mean?}}

Finally, let's run an IV regression

\textbf{\textsc{Does conditioning on covariates improve precision?}}

\subsection{Estimating sampling
noise}\label{estimating-sampling-noise-3}

\textbf{\textsc{Derive CLT disrtribution }}
