...
The key idea is to recognize that, with a power of \(\kappa\) for a two-sided test, the distribution is a standard normal centered at \(MDE_{\kappa,\alpha}=\Phi^{-1}(\kappa)+\Phi^{-1}(1-\alpha/2)\).
...
Now, the probability that this distribution gives a p-value of 0.05 or smaller for a two-sided test of the true effect being zero with size 5\% is equal to \(\Phi(MDE_{\kappa,\alpha}-\Phi^{-1}(1-\alpha/2))=\kappa\) by definition.
The probability that it gives a p-value of \(p\) for the same test is of \(\Phi(MDE_{\kappa,\alpha}-\Phi^{-1}(1-p/2))\).
...
\begin{align*}
  pp_{\kappa,\alpha}(p_1,p_2) & = \frac{1}{\kappa}\left(\Phi(MDE_{\kappa,\alpha}-\Phi^{-1}(1-p_2/2))-\Phi(MDE_{\kappa,\alpha}-\Phi^{-1}(1-p_1/2))\right).
\end{align*}
...
{\centering \includegraphics[width=0.5\linewidth]{STCI_files/figure-latex/pCurvePlot-1} 
...
\hypertarget{correction-for-qrps-and-publication-bias-using-p-curving}{%
\paragraph{Correction for QRPs and publication bias using p-curving}\label{correction-for-qrps-and-publication-bias-using-p-curving}}
