...
The Funnel Asymmetry Test and the Precision-Effect Test are based on the following meta-regression:
...
The Funnel Asymmetry Test tests the assumption that \(\alpha_1=0\) using a standard two-sided t-test.
Rejecting the null means that there is sign of publication bias.
The Precision-Effect Test tests whether \(\alpha_0=0\).
Rejecting the null means that there is evidence of a true effect.

\BeginKnitrBlock{example}
\protect\hypertarget{exm:unnamed-chunk-166}{}{\label{exm:unnamed-chunk-166} }Let's see in practice how FAT and PET work in our example.
\EndKnitrBlock{example}
...
When running the regression on the whole sample, in the absence of publication bais, we find that the estimated coefficient for the standard error in the meta-analytic regression is -0.34 \(\pm\) 0.95.
As a consequence, there is no sign of publication bias, with a pretty decent precision level.
...
The coefficient is positive, as expected if larger results occur with smaller sample size, but the precision of this coefficient is too low to be able to detect publication bias.
This is a characteristic of the Funnel Asymetry Test to have low power, especially in our case where only one observation with small sample size drives all the results.

The Precision-Effect Test detects a positive effect in the absence of publication bias (0.28 \(\pm\) 0.12) that is significantly different from zero.
But, following the practice suggested by Docouliagos and Stanley, we should refrain from using this estimate and focus only on the simple meta-analytic one, since the Funnel Asymmetry Test has not detected signs of publication bias.
The Precision-Effect Test does not detect a positive effect in the presence of publication bias (0.14 \(\pm\) 0.29).
Again, following the practice suggested by Docouliagos and Stanley, we should refrain from using this estimate and focus only on the simple meta-analytic one, since the Funnel Asymmetry Test has not detected signs of publication bias.

PEESE + graph with curves + RE
