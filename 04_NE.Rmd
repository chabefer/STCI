...
#### Reverse DID designs when everyone enters the treatment at the second period

Compared to the setting in the previous section, the main change is to Assumption \@ref(hyp:NoTreatmentFirst):

```{hypothesis,AllTreatmentSecond,name="Everyone Receives Treatment in the After Period"}
We assume that every unit in the population receives the treatment in the After period: $D_{i,A}=1$, $\forall i$.
```
...
```{remark}
Note finally that in this model, the $DID^r$ estimator is still biased for $\Delta^{Y_B}_{TUT}$.
The reasons why are left as an exercise.
```

**final example: no change in treatment effects over time**



#### Reverse DID designs where everyone is in the treatment at the first period

Compared to the setting in the previous section, the main change is to Assumption \@ref(hyp:NoTreatmentFirst):

```{hypothesis,AllTreatmentFirst,name="Everyone Receives Treatment in the Before Period"}
We assume that every unit in the population receives the treatment in the Before period: $D_{i,B}=1$, $\forall i$.
```

Under Assumption \@ref(hyp:AllTreatmentFirst), and without loss of generality, we can still write $D_i=D_{i,A}$.
A consequence of Assumptions \@ref(hyp:AllTreatmentFirst) and \@ref(hyp:NoAnticipationEffects) is that we can write observed outcomes as a function of treatment and potential outcomes using the usual switching equation (\@ref(eqn:switchDID)).

```{example}
Let us generate data in our example model that complies with Assumption \@ref(hyp:AllTreatmentFirst).
```

\begin{align*}
y_{i,A}^1 & = y_{i,A}^0+\bar{\alpha}_A+\theta_A\mu_i+\eta_{i,A} \\
y_{i,A}^0 & = \mu_i+\delta+U_{i,A}^0 \\
U_{i,A}^0 & = \rho U_{i,B}+\epsilon_{i,A} \\
y^1_{i,B} & =y^0_{i,B} + \bar{\alpha}_B+\theta_B\mu_i+\eta_{i,B} \\
y^0_{i,B} & =\mu_i+U_{i,B} \\
U_{i,B} & \sim\mathcal{N}(0,\sigma^2_{U}) \\
D_{i,A}   & = \uns{y^0_{i,A}-\delta+ V_i\leq\bar{y}} \\
V_i   & = \gamma(\mu_i-\bar{\mu}) + \omega_i \\
(\eta_{i,A},\eta_{i,B},\omega_i) & \sim\mathcal{N}(0,0,0,\sigma^2_{\eta},\sigma^2_{\eta},\sigma^2_{\omega},0,\rho_{\eta,\omega})
\end{align*}


```{r param.DID.reverse,eval=TRUE,echo=TRUE,results='hide'}
param <- c(8,.5,.28,1500,0.9,0.01,0.01,0.05,0.05,0.05,0.5,0.1,0.1,0.28,0)
names(param) <- c("barmu","sigma2mu","sigma2U","barY","rho","thetaA","thetaB","sigma2epsilon","sigma2eta","delta","baralphaA","baralphaB","gamma","sigma2omega","rhoetaomega")
```

```{r SimulDIDRev,eval=TRUE,echo=TRUE,results='hide'}
set.seed(1234)
N <- 1000
cov.eta.omega <- matrix(c(param["sigma2eta"],0,param["rhoetaomega"]*sqrt(param["sigma2eta"]*param["sigma2omega"]),
                          0,param["sigma2eta"],param["rhoetaomega"]*sqrt(param["sigma2eta"]*param["sigma2omega"]),
...
U0A <- param["rho"]*UB + epsilonA
y0A <- mu +  U0A + param["delta"]
alphaA <- param["baralphaA"]+  param["thetaA"]*mu + eta.omega$etaA
y1A <- y0A+alphaA
Y0A <- exp(y0A)
Y1A <- exp(y1A)
Ds <- rep(0,N)
V <- param["gamma"]*(mu-param["barmu"])+eta.omega$omega
Ds[y0A-param["delta"]+V<=log(param["barY"])] <- 1 
yA <- y1A*Ds+y0A*(1-Ds)
YA <- Y1A*Ds+Y0A*(1-Ds)
yB <- y1B
YB <- Y1B
...
```{r PlotDIDRev,eval=TRUE,fig.cap='Evolution of average outcomes in the always treated and switchers group in the reverse DID design',fig.align='center',out.width='50%',echo=TRUE,results='hide',fig.pos='htbp'}
x <- c("Before","After")
y.AT <- c(mean(yB[Ds==1]),mean(yA[Ds==1]))
y.Switchers <- c(mean(yB[Ds==0]),mean(yA[Ds==0]))
y.Switchers.counterfactual <- c(mean(y1B[Ds==0]),mean(y1A[Ds==0]))
y.Switchers.DID <- c(mean(yB[Ds==0]),mean(yB[Ds==0])+mean(yA[Ds==1])-mean(yB[Ds==1]))
data.DID.plot <- as.data.frame(c(y.AT,y.Switchers,y.Switchers.counterfactual,y.Switchers.DID))
colnames(data.DID.plot) <- c("Outcome") 
data.DID.plot$Period <- factor(rep(x,4),levels=c("Before","After"))
data.DID.plot$Group <- factor(c("Always Treated","Always Treated","Switchers","Switchers","Switchers counterfactual","Switchers counterfactual","Switchers DID","Switchers DID"),levels=c("Switchers counterfactual","Switchers DID","Switchers","Always Treated"))
data.DID.plot$Observed <- factor(c("Observed","Observed","Observed","Observed","Unobserved","Unobserved","Observed","Observed"),levels=c("Observed","Unobserved"))
...
Counterfactual.after <- mean(yB[Ds==0])+BA.AT
DID <- WW.after - WW.before 
TTA <- mean(alphaA[Ds==0])
TTB <- mean(alphaB[Ds==0])
...
In this section, we are looking at what happens 

**Build a new dataset with several time periods.**

#### Identification

The first thing is that, with several periods after the treatment date, we now have a time-varying $TT_t$: $\Delta^{TT}_{Y_t}=\esp{Y^1_{i,t}-Y^0_{i,t}|D_i=1}$.
A key question is whether some or all of the estimators we have listed above continue to estimate an interesting combination of the $\Delta^{TT}_{Y_t}$ parameters and under which assumptions.
Another key question is whether alternative estimators are needed, which ones we can build and how they behave under which types of assumptions.
We are going to first describe the setting and the main assumptions in this more general case before stating our first result, which is going to be negative: under fairly general assumptions, the Two-Way Fixed-Effects estimator does not recover an interesting weighted average of the $TT_t$.
We will examine the reasons for this failure and then list the possible alternatives to this estimator that have been proposed in the literature so far.
