...
```{theorem,asympnoiseDID,name="Asymptotic Estimate of Sampling Noise of DID"}
Under Assumptions \@ref(hyp:NoTreatmentFirst), \@ref(hyp:NoAnticipationEffects), \@ref(hyp:ParallelTrends), \@ref(hyp:iidDID) and \@ref(hyp:finitevarDID), for a given confidence level $\delta$ and sample size $N$, the sampling noise of $\hat{\Delta}^Y_{DID}$ can be approximated as follows:

\begin{align*}
2\epsilon^{panel}_{DID} & \approx 2\Phi^{-1}\left(\frac{\delta+1}{2}\right)\frac{1}{\sqrt{N}}\sqrt{\frac{\var{Y_{i,A}^1-Y_{i,B}^0|D_i=1}}{\Pr(D_i=1)}+\frac{\var{Y_{i,A}^0-Y_{i,B}^0|D_i=0}}{1-\Pr(D_i=1)}} \equiv 2\tilde{\epsilon}^{panel}_{DID}.
\end{align*}
...
```{theorem,asympnoiseDIDCross,name="Asymptotic Estimate of Sampling Noise of DID in Repeated Cross Sections"}
Under Assumptions \@ref(hyp:NoTreatmentFirst), \@ref(hyp:NoAnticipationEffects), \@ref(hyp:ParallelTrends), \@ref(hyp:iidDIDCross) and \@ref(hyp:finitevar), for a given confidence level $\delta$, the sampling noise of $\hat{\Delta}^Y_{DID}$ in repeated cross sections of total size $N$ can be approximated as follows:

\begin{align*}
2\epsilon^{cross}_{DID} & \approx 2\Phi^{-1}\left(\frac{\delta+1}{2}\right)\frac{1}{\sqrt{N}}\sqrt{
                            \frac{\var{Y^0_{i,B}|D_i=0}}{(1-p)(1-p_A)}
                            +\frac{\var{Y^0_{i,B}|D_i=1}}{p(1-p_A)}
                            +\frac{\var{Y^0_{i,A}|D_i=0}}{(1-p)p_A}
                            +\frac{\var{Y^1_{i,A}|D_i=1}}{pp_A}} \\
                        & \equiv 2\tilde{\epsilon}^{cross}_{DID}.
\end{align*}
...
The estimated level of 99\% sampling noise obtained using the heteroskedasticity robust standard errors from the DID regression using OLS is equal to `r round(SN.DID.estim.OLS.1000,2)`.
The `plm` package returns an estimate of 99\% sampling noise equal to `r round(SN.DID.estim.OLS.plm.1000,2)` for the pooled DID estimator.
