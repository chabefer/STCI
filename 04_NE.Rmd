...
##### Using weighted averages of individual DID estimators
...
We then can aggregate the estiamted terms.
Let's go:
...
In order to do that, we need to be able to access the individual coefficeints for each treatment group, and then to combine them.
One way to do that is to we need to define a time to treatment indicator (this is the role of $\tau=t-d$ in the above equation) and to run a TWFE regression with individual and year fixed effects, and one interacted dummy per time to treatment and treatment group. 
Note that there has to be a reference time to treatment (here $\tau=-1$ (note that $\tau$ can take negative values)) and a reference treated group (here $D_i=\infty$ (in practice, $D_i=99$)).
In practice, we are generating a variable of time to treatment which takes value $-99$ fot the never treated and we exclude this value and the $-1$ time to treatment date. 

```{r DIDSAfixestMore,eval=TRUE,warning=FALSE,error=FALSE,message=FALSE,echo=TRUE}
...
**Aggregate above using a weighting matrix**
**plot for all groups**
**plot with constant group composition**
