...
In the sample, Sun and Abraham's estimator (estimated by OLS in repeated cross sections, by OLS using the Least Squares Dummy Variables model, or following the Within transformation or the First Difference transformation in panel data) is equal to the individual DID estimators using the never treated as the comparison group and the period just before receiving the treatment as the reference period: $\forall d\in\left\{2,\dots,T\right\}$, $\forall\tau\in\left\{-(T-1),\dots,T-2\right\}\setminus\left\{-1\right\}$,
...
There are two sets of parameters for which we might want to know their distribution: the parameter specific to each treated group and relative time to treatment $\hat\beta^{SA}_{d,\tau}$ and the aggregated treatment effect $\hat\delta^{Y}_TT(k)$ for some set of weights $w^k(d,d',\tau,\tau')$.
