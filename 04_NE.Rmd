...
We can define a separate treatment effect for each of the treatment groups and for each time period: $\Delta^{Y_t}_{TT_d}=\esp{Y^1_{i,t}-Y^0_{i,t}|D_i=d}$, for $t,d\in\left\{1,2,\dots,T\right\}$.
We can also define a treatment on the untreated $\Delta^{Y_t}_{TUT}=\esp{Y^1_{i,t}-Y^0_{i,t}|D_i=\infty}$.

We can also form a very large bunch of DID estimators: 

\begin{align*}
  \Delta^{Y}_{DID}(d,d',\tau,\tau') & = \esp{Y_{i,d+\tau}|D_i=d}-\esp{Y_{i,d-\tau'}|D_i=d}-\esp{Y_{i,d+\tau}-Y_{i,d-\tau'}|D_i=d'},
\end{align*}
...
If $d-\tau'<d'<d+\tau$, $\Delta^{Y}_{DID}(d,d',\tau,\tau')$ is not a proper DID estimator since $d'$ receives also receives the treatment between the two dates at which outcomes are measured.
...
Since this estimator is a $DID^r$ estimator, I am going to note it as such in the future.
...
\begin{align*}
    \esp{Y^0_{i,t}|D_i=d} - \esp{Y^0_{i,t'}|D_i=d} & =   \esp{Y^0_{i,t}|D_i=\infty} - \esp{Y^0_{i,t'}|D_i=\infty}.
\end{align*}
$\forall d\in\left\{1,2,\dots,T\right\}$ and $\forall t,t'\in\left\{1,2,\dots,T\right\}$
```
...
Under Assumptions \@ref(hyp:NoTreatmentTime), \@ref(hyp:NoAnticipationEffectsTime) and \@ref(hyp:ParallelTrendsTime), the DID estimator identifies the average effect of the Treatment on the Treated after the treatment in each time period:

\begin{align*}
    \Delta^{Y}_{DID}(d,d',\tau,\tau') & =  \Delta^{Y_{d+\tau}}_{TT_d},
\end{align*}
where $\tau,\tau'>0$ and $d'>d+\tau$. 
```
...
\begin{align*}
  \Delta^{Y}_{DID}(d,d',\tau,\tau') & = \esp{Y_{i,d+\tau}|D_i=d}-\esp{Y_{i,d-\tau'}|D_i=d}-\esp{Y_{i,d+\tau}-Y_{i,d-\tau'}|D_i=d'}\\
                                    & = \esp{Y^1_{i,d+\tau}|D_i=d}-\esp{Y^0_{i,d-\tau'}|D_i=d}-\esp{Y^0_{i,d+\tau}-Y^0_{i,d-\tau'}|D_i=d'}\\
                                    & = \esp{Y^1_{i,d+\tau}|D_i=d}-\esp{Y^0_{i,d-\tau'}|D_i=d}-\esp{Y^0_{i,d+\tau}-Y^0_{i,d-\tau'}|D_i=d}\\
