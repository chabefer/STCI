...
```{r SimulDIDPT,eval=TRUE,echo=FALSE,results='hide'}
set.seed(1234)
N <-1000
param["rho"] <- 1
cov.eta.omega <- matrix(c(param["sigma2eta"],param["rhoetaomega"]*sqrt(param["sigma2eta"]*param["sigma2omega"]),param["rhoetaomega"]*sqrt(param["sigma2eta"]*param["sigma2omega"]),param["sigma2omega"]),ncol=2,nrow=2)
...
##### Using the Least Squares DID estimator
...
The way it works is as follows: estimate the following equation using OLS and use $\hat{\delta}_{OLS}$ as your DID estimate: $\hat{\delta}_{OLS}=\hat{\Delta}^{Y}_{DID}$. 
\begin{align*}
  	Y_i &  = \alpha +  \beta D_i + \gamma T_i + \delta D_iT_i + \epsilon_i.
\end{align*}
...
```{r RegDID,eval=TRUE,echo=FALSE,results='hide'}
...
The estimate of $\hat{\delta}_{OLS}$ in our sample is equal to `r round(DID.est.reg,3)`.
...
$\beta$, the coefficient in front of the $D_i$ dummy, estimates the With/Without estimator before the treatment.
In our case, we have $\hat{\beta}_{OLS}=$ `r round(WW.before.reg,3)`.
...
```{r RegFD,eval=TRUE,echo=FALSE,results='hide'}
# building a data frame
...
One very computer-intensive way to estimate $TT$ in a DID setting is to use the OLS estimator supplemented with dummies for each observation and for each time period.
In practice, the estimator is based on the following regression:
\begin{align*}
  	Y_{i,t} &  = \sum_{j=1}^N\mu_j\uns{j=i} + \sum_{l=0}^1\delta_l\uns{l=t} + \beta^{TWFE} D_{i,t} + \epsilon^{TWFE}_i.
\end{align*}
This estimator is generally simplified as follows:
\begin{align*}
  	Y_{i,t} &  = \mu_i + \delta_t + \beta^{TWFE} D_{i,t} + \epsilon^{TWFE}_i,
\end{align*}
and called the Two-Way Fixed Effects estimator, since it has two-sets of so-called fixed effects (individual fixed effects, $\mu_i$, and time fixed effects $\delta_t$).
I will write it using this second, more compact, formulation, but I thinkt he first formulation encapsulates better how the Least-Squares Dummy Variables estimator works.
In what follows, we will 


```{remark}
The term *fixed effect* is specific to the panel data literature in econoemtrics. 
...
##### Using the Two-Way Fixed Effects estimator

Finally, a more universal approach is to rely on the Two-Way Fixed Effects estimator.
There are multiple ways to implement it.
`R` proposes the wonderful `fixest` package by Laurent Bergé, which works even with a very large number of fixed effects.
It does not work by differencing the the data with respect to the grouped-means, as the traditional within estimator does (see `plm` for an implementation of that approach).
`fixest` uses the concentrated likelihood in order to estimate the treatment effect.
The algorithm works by first estimating the fixed effects and then by running the likelihood on the differenced out data. (?)

#### Inference

Inference from the OLS with group fixed effects will be too wide.
