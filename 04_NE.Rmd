...
# regression
reg.dCdH <- did_multiplegt_old(Y="y",T="time",G="id",D="D",df=filter(data,Ds!=1),placebo=3,dynamic=3,average_effect="prop_number_switchers")
```
...
# regression
reg.BJS <- did_imputation(yname="yBJS",tname="time",idname="id",gname="Group",data=filter(data,Ds!=1),horizon=TRUE,pretrends=TRUE)
```
...
# regression
reg.BJS.Agg <- did_imputation(yname="yBJS",tname="time",idname="id",gname="Group",data=filter(data,Ds!=1))
```

The treatment effect estimated using Borusyak, Jaravel and Speiss's estimator is equal to `r round(reg.BJS.Agg[[3]],2)` $\pm$ `r round(1.96*reg.BJS.Agg[[4]],2)`.
...
##### Estimating sampling noise using de Chaisemartin and d'Haultfoeuille's approach

The key element in [de Chaisemartin and d'Haultfoeuille](https://ssrn.com/abstract=3731856)'s approach to estimating sampling noise allowing for heteroskedasticity is to model heteroskedasticity as being a function of cohort membership.
...
More formally, they index the set of cohorts by $k$, where $k$ maps all the possible values of these three parameters: 
