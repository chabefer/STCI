...
Let us start by projecting outcomes and remaining regressors on the first regressor (the constant), using the projection matrix $M_1$ and denoting $X_1$ the first column of hte $X$ matrix.
The residualized outcome is equal to:

\begin{align*}
	M_1Y &  = Y-X_1(X_1'X_1)^{-1}X_1'Y \\
	     & = Y-\left(\begin{array}{c} 1 \\	\vdots \\	1 \\1 \\	\vdots \\	1 \end{array}\right)
	     \left(
	     \left(\begin{array}{cccccc} 1 & \hdots &	1 & 1 &	\hdots &	1 \end{array}\right)
	     \left(\begin{array}{c} 1 \\	\vdots \\	1 \\1 \\	\vdots \\	1 \end{array}\right)
	     \right)^{-1}
	     \left(\begin{array}{cccccc} 1 & \hdots &	1 & 1 &	\hdots &	1 \end{array}\right)
	     Y \\
	     & = Y-	\frac{1}{2N}\left(\begin{array}{cccccc}
	                    1 & \hdots &	1 & 1 &	\hdots &	1 \\
	                    ...
	                    1 & \hdots &	1 & 1 &	\hdots &	1
	                  \end{array}\right) Y \\
	     & = \left(\begin{array}{c}  Y_{1,B}-\frac{1}{2}\left(\frac{1}{N}\sum_{i=1}^NY_{i,B}+\frac{1}{N}\sum_{i=1}^NY_{i,A}\right) \\	\vdots \\
	                                 Y_{N,B}-\frac{1}{2}\left(\frac{1}{N}\sum_{i=1}^NY_{i,B}+\frac{1}{N}\sum_{i=1}^NY_{i,A}\right) \\
	                                 Y_{1,A}-\frac{1}{2}\left(\frac{1}{N}\sum_{i=1}^NY_{i,B}+\frac{1}{N}\sum_{i=1}^NY_{i,A}\right) \\	\vdots \\
	                                 Y_{N,A}-\frac{1}{2}\left(\frac{1}{N}\sum_{i=1}^NY_{i,B}+\frac{1}{N}\sum_{i=1}^NY_{i,A}\right) \end{array}\right),
\end{align*}

which simply is the deviation of each individual outcome from the sample mean.

Using the same approach, we have:

\begin{align*}
	M_1D & = \left(\begin{array}{c}  D_{1}-\frac{1}{N}\sum_{i=1}^ND_{i} \\	\vdots \\
	                                 D_{N}-\frac{1}{N}\sum_{i=1}^ND_{i} \\
	                                 D_{1}-\frac{1}{N}\sum_{i=1}^ND_{i} \\	\vdots \\
	                                 D_{N}-\frac{1}{N}\sum_{i=1}^ND_{i} \end{array}\right),
\end{align*}

\begin{align*}
	M_1T & = \left(\begin{array}{c}  -\frac{1}{2} \\	\vdots \\
	                                 -\frac{1}{2} \\
	                                 \frac{1}{2} \\	\vdots \\
