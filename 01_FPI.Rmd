```{r libraries,echo=FALSE,message=FALSE,error=FALSE}
library(xtable)
```
...
You can see on Figure \@ref(fig:histyb) a histogram of $y_i^B$ with the red line indicating the cutoff point: $\bar{y}=\ln(\bar{Y})=$ `r round(log(param["barY"]),2)`.
...
We can see on Table \@ref(tab:table.D.sharp) that there are `r table.D.sharp[which(names(table.D.sharp)==1)]` treated observations.
