...
```{proof}
See in appendix \@ref(sec:proofcheb)
```
...
```{r sampnoisewwcheball,eval=TRUE,echo=TRUE,fig.cap='Distribution of the Chebyshev upper bound on sampling noise over replications of samples of different sizes (true sampling noise in red)',fig.align='center',out.width=cst,fig.pos='htbp',fig.height=12,fig.width=10}
for (k in (1:length(N.sample))){
...
A downside of Chebyshev sampling noise is their lower precision, which makes it harder to pinpoint the true confidence intervals.

```{r sampnoisewwchebplot,eval=TRUE,echo=TRUE,fig.cap='Average Chebyshev upper bound on sampling noise over replications of samples of different sizes (true sampling noise in red)',fig.align='center',out.width=cst,fig.pos='htbp',fig.height=12,fig.width=10}
for (k in (1:length(N.sample))){
...
### Using randomization inference {#sec:Fisher}
