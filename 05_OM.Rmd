...
```{remark}
Theorem \@ref(thm:OLSWeights) shows that OLS without interactions between the treatment and the covariates is a weighted average of the effect of the treatment on the treated ($TT$) and of the effect of the treatment on the untreated ($TUT$).
...
Let us first redefine the LLR function:

```{r LLRMatch,eval=TRUE,echo=TRUE}
...
MSE.grid.test <- sapply(MSE.grid,MSE.llr,y=y,D=Ds,x=pscore,kernel='gaussian')
