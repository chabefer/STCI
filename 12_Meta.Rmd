...
```{r metanoiseplot,eval=TRUE,echo=TRUE,results='hide',warning=FALSE,error=FALSE,message=FALSE,fig.cap='Datasets with treatment effect heterogeneity',fig.subcap=c('initial',paste('$\\tau^2=$',tau[[1]],sep=' '),paste('$\\tau^2=$',tau[[2]],sep=' ')), fig.align='center', out.width="33%", fig.pos='htbp', fig.height=8, fig.width=12}
  ...
  ggplot(data.meta, aes(x=as.factor(id), y=theta.1)) +
      ...
      geom_hline(aes(yintercept=ES(param)), colour="#990000", linetype="dashed")+
      xlab(paste('Studies','$\\tau^2=$',tau[[1]],sep=' '))+
      ylab("Effect size")+
      theme_bw()+
      ylim(-2,2)
  ...
  ggplot(data.meta, aes(x=as.factor(id), y=theta.2)) +
      ...
      geom_hline(aes(yintercept=ES(param)), colour="#990000", linetype="dashed")+
      xlab(paste('Studies','$\\tau^2=$',tau[[2]],sep=' '))+
      ylab("Effect size")+
      theme_bw()+
      ylim(-2,2)
```
...
Hedges then proposes a new estimator for the average effect of the treatment, an estimator that accounts for the additional noise due to heterogeneous treatment effects accross sites.
...
```
Hedges' estimates of the average effect size is equal to `r round(Hwmae(data.meta$theta.1,data.meta$se.ES^2,tau.2.theta.1),2)` and  `r round(Hwmae(data.meta$theta.2,data.meta$se.ES^2,tau.2.theta.2),2)` respectively, while the true value is `r round(ES(param),2)`.

**<span style="font-variant:small-caps;">Show other estimates than $\tau^2$.</span>**

**<span style="font-variant:small-caps;">Show other estimators than Hedges' and how to implement them with `rma`.</span>**
