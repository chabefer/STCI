...
**<span style="font-variant:small-caps;">Another approach coud use the distance between the observed pp-curve of the previous section and the pp-curve with various levels of power. Once the power is identified, the effect size is identified. Still another approach would be to use the standardized distribution of effects (distributed as a standardized normal) to estimate the mean of the distribution and then recover the treatment effect. </span>**
...
ppCurveEstES <- function(thetak,sigmak,thetacl,thetach,alpha=0.05,ngrid=100){
  ...
  # compute sht KS stat for each value of thetac (over columns)
  ...
  return(list(min.theta.c,thetac.grid,KS.grid,ecdf(ppCurve.grid[,match(min(KS.grid),KS.grid)])))
}

test <- ppCurveEstES(thetak=c(4,2,3,1),sigmak=c(1,1,1,0.5),thetacl=0,thetach=10,alpha=0.05,ngrid=100)
