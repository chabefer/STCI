...
The FAT estimate of the coefficient for the standard error in the meta-analytic regression is `r round(coef(FAT.PET.RE.interm.pubbias)[2],2)` $\pm$ `r round(qnorm((delta.2+1)/2)*summary(FAT.PET.RE.interm.pubbias)$se[2],2)`. 
The coefficient is positive, as expected if larger results occur with smaller sample size, but the precision of this coefficient is too low for the FAT to be able to detect publication bias.
