...
#### Estimating the average effect of the treatment taking heterogeneity into account 
...
where $X_i$ is a vector of variables suspected to be correlated with treatment effect heterogeneity and $\beta$ is a vector of coefficients, of the same dimension as $X_i$.
$X_i$ contains a $1$ as its first term, so that $\beta_0$, the first component of the vector $\beta$ measures the effect of the treatment when all other regressors are set to zero.
...
The error terms $u_i$ and $e_i$ capture the remaining heterogeneity in effect sizes due to remaining unobserved covariates ($u_i$) and estimation error due to sampling noise ($e_i$).
It is in addition generally assumed that $e_i\sim\mathbf{N}(0,\sigma^2_i)$ and  $u_i\sim\mathbf{N}(0,\tau^2)$.

This model is in general called the **mixed effects linear model**.
It contains at the same time fixed effects captured by $\beta'X_i$ and random effects captured by $u_i$.
