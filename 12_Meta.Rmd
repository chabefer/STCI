...
where $\epsilon_k$ is sampling noise, $\nu_k$ is the variation in effect size in the population and $\alpha$ is the average of the effect size accross all populations.
We denote, as is customary, the variance of $\nu_k$ as $\tau^2$.
...
Hedges' estimate of $\tau^2$  slightly negative, which is problem, since a variance is always positive.
...
We will present ehm succinctly since they are part of the `metafor` package.

Before that, let's create a new sample of effect sizes where we add noise to each estimate stemming not from sampling, but from heterogeneity in treatment effects across sites and studies.
...
```{r metanoiseplot,eval=TRUE,echo=TRUE,results='hide',warning=FALSE,error=FALSE,message=FALSE,fig.cap='Datasets with treatment effect heterogeneity',fig.subcap=c(paste('$\\tau^2=$',tau[[1]],sep=' '),paste('$\\tau^2=$',tau[[2]],sep=' ')), fig.align='center', out.width="50%", fig.pos='htbp', fig.height=8, fig.width=12}
  ...
  ggplot(data.meta, aes(x=as.factor(id), y=theta.2)) +
      geom_bar(position=position_dodge(), stat="identity", colour='black') +
      geom_errorbar(aes(ymin=theta.1-qnorm((delta.2+1)/2)*se.ES, ymax=theta.2+qnorm((delta.2+1)/2)*se.ES), width=.2,position=position_dodge(.9),color='blue') +
      geom_hline(aes(yintercept=ES(param)), colour="#990000", linetype="dashed")+
      xlab("Studies")+
      ylab("Effect size")+
      theme_bw()
...
Hedges' estimates of $\tau^2$ in our examples are thus `r round(tau.2.theta.1,2)` and  `r round(tau.2.theta.2,2)` respectively.
The second one is equal to the truth, which is `r tau[[2]]^2`.
The first one is still imprecise.

**<span style="font-variant:small-caps;">Show Hedges reweighted estimator of average treatment effect.</span>**

**<span style="font-variant:small-caps;">Show Hedges estimator of variance of treatment effect (actually very simple?).</span>**
