...
```{example}
Let's see in practice how FAT and PET work in our example.
```
...
```{r FATPETFE,dependson=c('monte.carlo'),eval=TRUE,echo=TRUE,results='markup',warning=FALSE,error=FALSE,message=FALSE}
...
When running the regression on the whole sample, in the absence of publication bais, we find that the estimated coefficient for the standard error in the meta-analytic regression is `r round(coef(FAT.PET.FE.interm)[2],2)` $\pm$ `r round(qnorm((delta.2+1)/2)*summary(FAT.PET.FE.interm)$se[2],2)`. 
...
The Precision-Effect Test detects a positive effect in the absence of publication bias (`r round(coef(FAT.PET.FE.interm)[1],2)` $\pm$ `r round(qnorm((delta.2+1)/2)*summary(FAT.PET.FE.interm)$se[1],2)`) that is significantly different from zero.
...
The Precision-Effect Test does not detect a positive effect in the presence of publication bias (`r round(coef(FAT.PET.FE.interm.pubbias)[1],2)` $\pm$ `r round(qnorm((delta.2+1)/2)*summary(FAT.PET.FE.interm.pubbias)$se[1],2)`).
...
Remember that the simple meta-analytic effect estimated using only the statistically significant studies is equal to `r round(coef(meta.example.FE.interm.pubbias),2)` $\pm$ `r round(qnorm((delta.2+1)/2)*summary(meta.example.FE.interm.pubbias)$se,2)`, while the effect estimated all the 17 studies with a small and intermediate sample size is `r round(coef(meta.example.FE.interm),2)` $\pm$ `r round(qnorm((delta.2+1)/2)*summary(meta.example.FE.interm)$se,2)`.
