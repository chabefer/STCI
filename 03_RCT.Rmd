...
In order to state the identification results in the Randomization After Eligibility design rigorously, I need to define new potential outcomes:
  ...
  1. $Y_i^{d,r}$ is the value of the outcome $Y$ when individual $i$ belongs to the program group $d$ ($d\in\left\{0,1\right\}$) and has been randomized in group $r$ ($r\in\left\{0,1\right\}$).
  2. $D_i^r$ is the value of the program participation decision when individual $i$ has been assigned randomly to group $r$.

#### Identification of TT

In a Randomization After Eligiblity, we need three assumptions to ensure identification of the TT:
...
We can now see that the ITE is composed of two terms: the first term captures the effect of annoucing eligibility on those who decide to participate into the program; the second term captures the effect of announcing eligibility on those who do not participate into the program.
...
In an Encouragement Design, we can recover the causal effect of the treatment not on all the treated but on those treated whose participation into the program has been triggered by the encouragement.
These individuals reacting to the encouragement by participating in the program are usually called compliers.
The effect of the treatment on the compliers is called the Local Average Treatment Effect.
The main identification for Encouragement designs is that a Wald ratio (an IV estimator) recovers the LATE.
...
```{proof}
Under Assumptions \@ref(def:RandEncouragValid), \@ref(def:IndepEncourag) and \@ref(def:ExclRestr), we have:
...
where the second equality uses Assumption \@ref(def:Mono) and the definition of $LATE$.
...
where we have used Assumptions \@ref(def:RandEncouragValid) and \@ref(def:IndepEncourag).
...
```{remark}
The intuitution for the proof of Theorem \@ref(thm:IdentLATE) is as follows: 
```  

  - In the treatment arm (with $R_i=1$), everyone receives the encouragement to enter the program.
  Only always takers and compliers actually take the program, and we thus see $Y^1_i$ for them.
  Never takers and defiers do not take the program, and we thus see $Y^0_i$ for them.
  - In the control arm (with $R_i=0), no one receives the encouragement.
  Only always takers and defiers take the program, and we thus see $Y^1_i$ for them.
  Never takers and compliers do not take the program, and we thus see $Y^0_i$ for them.
  - When comparing the average outcomes between the treatment and control arms, we thus have two terms that cancel, since they are the same on both arms: the outcomes of the always treated and the outcomes of the never treated.
  We are thus left with the impact on the compliers minus the impact on the defiers, both weighted by theirn proportion in the population.
  If there are no defiers, we see the impact on the compliers multiplied by their proportion.
  - The difference between the proportion of participants in the treatment and control arms estimates the proportion of compliers minus the proportion of defiers.
  Under monotonicity, the proportion of defiers is zero, and thus the difference between the proportion of participants in the treatment and control arms estimates the proportion of compliers.
  
```{remark}
In the proof of Theorem \@ref(thm:IdentLATE), I use the types random variable $T_i$ instead of the potential outcomes $D^1_i$ and $D^0_i$.
This is equivalent, since $T_i$ is a function of $D^1_i$ and $D^0_i$.
Assumption \@ref(def:IndepEncourag) especially is also true when replacing the potential outcomes $D^1_i$ and $D^0_i$ by $T_i$.
This follows from Lemma 4.2 in Dawid (1979) that shows that if $X\Ind Y|Z$ and $U$ is a function of $X$ then $U\Ind Y|Z$.
```

```{remark}
The intuition for Theorem \@ref(thm:IdentLATE) is that the encouragement only generates the entry of compliers in the program.
We are thus only able to recover the effect of the treatment on the compliers.
The experimental variation is not strong enough so as to enable us to estimate the impact on always takers or on never takers.
```

```{remark}
The problem with LATE is that it is not a very useful parameter for evaluating the treatment.
LATE is the impact of the treatment on an unobserved subpopulation of individuals.
...
**<span style="font-variant:small-caps;">Develop intuition of ITE rigorously as in RAE. </span>**
