...
Under Monoticity, the variance of the Wald estimator can be decomposed as follows:
...
```
Without conditioning on $y_i^B$:

  - True 99\% sampling noise (from the simulations) is `r round(2*quantile(abs(simuls.encourage[['1000']][,'Wald']-delta.y.late),probs=c(0.99)),3)`.
  - 99\% sampling noise estimated using default 2SLS standard errors is `r round(2*qnorm((.99+1)/2)*sqrt(vcov(reg.y.R.2sls.encourage)[2,2]),3)`
  - 99\% sampling noise estimated using heteroskedasticity robust 2SLS standard errors is `r round(2*qnorm((.99+1)/2)*sqrt(vcovHC(reg.y.R.2sls.encourage,type='HC2')[2,2]),3)`.

With conditioning on $y_i^B$:

  - True 99\% sampling noise (from the simulations) is `r round(2*quantile(abs(simuls.encourage.yB[['1000']][,'Wald']-delta.y.late),probs=c(0.99)),3)`.
  - 99\% sampling noise estimated using default 2SLS standard errors is `r round(2*qnorm((.99+1)/2)*sqrt(vcov(reg.y.R.yB.2sls.encourage)[2,2]),3)`
  - 99\% sampling noise estimated using heteroskedasticity robust 2SLS standard errors is `r round(2*qnorm((.99+1)/2)*sqrt(vcovHC(reg.y.R.yB.2sls.encourage,type='HC2')[2,2]),3)`.
