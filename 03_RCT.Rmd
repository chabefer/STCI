...
The assumptions needed for that to happen are intuitive: 
  ...
  3. The encouragement does not generate two-way flows in and out of the treatment, but only one flow in (Assumption \@ref(def:Mono))
  
Under these assumptions, the only way that we see a difference in outcomes between those that receive the encouragement and those that do not is that the treatment has had an effect on those that have taken it because of the encouragement.
...
```
  ...
  They probably do not, since the expected benefits of the treatment might enter in the decision of taking it up.
...
This is not warranted by the maths, but one can understand how a bayesian decision-maker may use the informatio on some subpopulation to infer what would happen in another.
...
Under Assumption \@ref(def:RandEncouragValid), receiving the encouragement has several impacts: 
  ...
  2. Some individuals (the defiers) decide to exit the treatment,
...
We can now see that ITE is composed of four terms:
  ...
  This is equal to the LATE under Assumption \@ref(def:ExclRestr), when there is no direct effect of the encouragement on outcomes.
...
All these effects are weighted by the respective proportions of types in the population.

ITE "contains" LATE, as we have already seen, but that can be made clearer:
...
\begin{align*}
  \Delta^Y_{ITE} & = \esp{Y_i^{1}-Y_i^{1}|T_i=a,E_i=1}\Pr(T_i=a|E_i=1)\nonumber\\
                ...
                & \phantom{=}+ \esp{Y_i^{0}-Y_i^{0}|T_i=n,E_i=1}\Pr(T_i=n|E_i=1),(\#eq:ITE3)
\end{align*}

which proves the result.
```

The previous theorem shows that Assumption \@ref(def:ExclRestr) shuts down any direct effect of ereceiving the encouragement on outcomes.
...
Hence, ITE is equal to the impact of the treatment on those who react to the encouragement: the compliers and the defiers, weighted by their respective proportions.
...
Under Assumptions \@ref(def:RandEncouragValid), \@ref(def:ExclRestr) and \@ref(def:Mono), ITE is equal to the LATE multiplied by the proportions of compliers in the population: 
...
The parts due to *always takers* and *never takers* cancel under Assumptions \@ref(def:IndepEncourag) and \@ref(def:ExclRestr).
But this cancelling out only happens in the population.
