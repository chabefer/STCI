...
It follows from Theorems \@ref(thm:ITERAE) and \@ref(thm:prRAE) that, under Assumptions \@ref(def:indelig) and \@ref(def:randeligvalid), the Bloom esimator is equal to the ITE divided by the propotion of agents taking up the program when eligible:
...
Estimating samploing noise in the Randomization After Eligibility design is similar to the estimation of sampling noise in the Encouragement design.
Since the Randomization After Eligibility design is a special case of the Encouragement design, I detail the estimation of sampling noise in both designs in Section \@ref(sec:design4).
...
```{lemma,WaldIV,name='Wald and Bloom are IV'}
