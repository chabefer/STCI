...
The unmediated treatment effect on the treated is equal to the direct effect of the treatment on each of the types multiplied by their proportion in the (treated) population, keeping the value of the mediator at its default value when the treatment is equalm to 0.
...
where the second and third inequalities follows from the switching equation.
...
\begin{align*}
  ...
  \Delta^Y_{WW|M_i=d} & = \esp{Y_i|D_i=1,M_i=d}-\esp{Y_i|D_i=0,M_i=d}.
\end{align*}
...
In the first step, we randomly allocate units either to the treatment or to the 
...
\begin{align*}
  ...
  R^2_i & \Ind(Y_i^{00},Y_i^{01},Y_i^{10},Y_i^{11},M^1_i,M^0_i)|M_i=1,D_i=1
\end{align*}
